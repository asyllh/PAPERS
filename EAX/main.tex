...
\begin{frontmatter}
\title{Evolutionary Methods for the Score-Constrained Strip-Packing Problem}
\author{Asyl L. Hawa}
\author{Rhyd Lewis}
...
\noindent Here, \eqref{eqn:allpacked} and \eqref{eqn:nointersect} state the requirement that every element must be in exactly one of the $k$ subsets. Equation~\eqref{eqn:feasible} then specifies that each subset $S_j \in \mathcal{S}$ must be feasible, where $\mathcal{F}$ is used to denote the set of all feasible subsets of elements in $\mathcal{I}$. The notion of feasibility is dependent on the specific constraint of the given problem.
...
\begin{figure}[H]	
	\centering
	\includestandalone[width=0.65\textwidth]{figures/scspp}
...
\begin{itemize}[leftmargin=*]
	\item Start of introduction stays the same, general equations for grouping problems, explain each condition, feasibility depends on the specific problem.
	\item For example, graph colouring, feasible independent sets, BPP no bin overfilled.
	\item Then go on to explain SCSPP (not SubSCP), give formal definition, state as a BPP.
	\item Follow up with lit review, TPP, CSP-SDCL, shorten TPP to a sentence.
	\item Use BP notation rather than strips.
	\item Remove SubSCP from introduction, put in AHC section.
