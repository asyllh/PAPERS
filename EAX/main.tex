...
\begin{itemize}[leftmargin=*]
	\item CMSA does not perform as well as EA, CMSA works better for smaller $n$ (100 items) and real instances.
	\item CMSA is able to find solution using the same number of bins as EA or fewer for more instances in real instance classes than artificial.
	\item For all six real instance classes, there is at least one instance (out of the 50) in which the best solution found by CMSA comprises fewer bins than the best soution found by EA (best of three recombination operators).
	\item In R55 instance class, in two of the instances CMSA is able to find a solution using $t$ bins, while the best solution found by the EA for these instances uses more than $t$ bins.
	\item In 10 of the 12 instance classes, CMSA is able to find solution using the same number of bins as EA solution for at least one of the 50 instances.
	...
	\item EA recombination operators and local search only applied to a two solutions/pair of partial solutions, can be executed quickly, worst case would be one item per bin so $n$ bins, but even then recombination operators are quick, and local search using AHC can be done in worst case $O(n^2)$ (see theorem), whereas exact cover/algorithm X is applied to the entire set $\mathcal{B}$ which in some cases contains up to 2800 bins, NPC.
	...
	\item if no solution found within 600 seconds, $\mathcal{S}^*$ automatically set to $\mathcal{S}_{\textup{bsf}}$. Again, could be that better solution exists, but not enough time to find it (unless first iteration of CMSA and $\mathcal{S}_{\textup{bsf}} = \emptyset$, then keep going until one solution found).
	\item Example, CMSA102 and CMSR102 for all 50 instances only XC during the first iteration of CMSA is able to find the best solution (min cardinality) within 600 seconds, but all subsequent iterations XC runs for the whole 600 seconds without finding a single solution, so $\mathcal{S}^*$ is set to $\mathcal{S}_{\textup{bsf}}$, and thus the average number of iterations are 7 and 6.98 respectively. In comparison, EA102 and ER102 run for 8603.66 and 5551.34 iterations respectively. This issue also occurs for CMSA52, CMSA105, CMSR52 and CMSR105.
	\item Pros: CMSA is easier to implement, simple setup, can use different method to generate solutions/bins for $\mathcal{B}$, i.e. AGX' which focuses on well packed bins, no duplicates in $\mathcal{B}$, whereas in EA could have two (or more) identical parents solutions.
	\item Removal of bins $\mathcal{B}$ after set period of time, having all bins in a pool, whereas could be bins in parent solutions in EA that remain after longer periods of time, in EA only two solutions are used from the population in each iteration, but CMSA uses entire pool.
\end{itemize}
}
