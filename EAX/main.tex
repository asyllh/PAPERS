...
\begin{tabular}{lrrcrrcrr}\toprule
	& & & & \multicolumn{2}{c}{MFFD} &\phantom{ab}& \multicolumn{2}{c}{MFFD$^+$}\\
	\cmidrule{5-6} \cmidrule{8-9}
	\multicolumn{1}{c}{Type, $W$} & \multicolumn{1}{c}{$|\mathcal{I}|$} & \multicolumn{1}{c}{$t$\tnote{$a$}} && \multicolumn{1}{c}{$|\mathcal{S}|$\tnote{$b$}} & \multicolumn{1}{c}{$\# t$\tnote{$c$}} && \multicolumn{1}{c}{$|\mathcal{S}|$} & \multicolumn{1}{c}{$\# t$}\\ \midrule	
...
artificial, 100 & 2500 & 23.323 & 30.754 & 0 && \textbf{28.457} & \textbf{26} \\
artificial, 500 & 2500 & 114.942 & 140.206 & 0 && \textbf{132.647} & 0 \\
artificial, 1000 & 2500 & 229.437 & 271.919 & 0 && \textbf{258.388} & 0 \\
\midrule
artificial, 100 & 5000 & 11.922 & 23.583 & 0 && \textbf{19.881} & \textbf{7} \\
artificial, 500 & 5000 & 57.722 & 103.209 & 0 && \textbf{89.544} & 0 \\
artificial, 1000 & 5000 & 114.965 & 198.325 & 0 && \textbf{172.613} & 0 \\
\midrule
\midrule
real, 100 & 2500 & 23.473 & 37.069 & 5 && \textbf{35.419} & \textbf{16} \\
real, 500 & 2500 & 115.239 & 184.106 & 0 && \textbf{177.249} & 0 \\
real, 1000 & 2500 & 229.946 & 368.453 & 0 && \textbf{355.042} & 0 \\
\midrule
real, 100 & 5000 & 11.981 & 32.348 & 1 && \textbf{29.611} & \textbf{5} \\
real, 500 & 5000 & 57.865 & 163.819 & 0 && \textbf{153.416} & 0\\
real, 1000 & 5000 & 115.227 & 328.618 & 0 && \textbf{308.642} & 0 \\
\bottomrule
\end{tabular}
\vspace{0.2cm} %
\begin{tablenotes}
...
\begin{tabular}{lrrcrrcrrcrr}
	...
	\multicolumn{1}{c}{Type, $W$} & \multicolumn{1}{c}{$|\mathcal{I}|$} & \multicolumn{1}{c}{$t$\tnote{$a$}} && \multicolumn{1}{c}{$|\mathcal{S}|$\tnote{$b$}} & \multicolumn{1}{c}{$\# t$\tnote{$c$}} && \multicolumn{1}{c}{$|\mathcal{S}|$} & \multicolumn{1}{c}{$\# t$} && \multicolumn{1}{c}{$|\mathcal{S}|$} & \multicolumn{1}{c}{$\# t$}\\
	\midrule
...
\cmidrule{5-6} \cmidrule{8-9} \cmidrule{11-12}
Type, $n$& $W$ & $t$\tnote{$a$} && $|\mathcal{S}|$\tnote{$b$} & $\# t$\tnote{$c$} && $|\mathcal{S}|$ & $\# t$ && $|\mathcal{S}|$ & $\# t$\\
\midrule
artificial, 100 & 2500 & 23.323 && 23.358 & 973 && \textbf{23.343} & \textbf{985} && 23.355 & 975 \\
artificial, 500 & 2500 & 114.942 && 116.44 & 316 && 116.711 & 280 && \textbf{116.326} &\textbf{347} \\
artificial, 1000 & 2500 & 229.437 && 234.482 & 14 && 235.029 & 17 && \textbf{233.988} &\textbf{21} \\
\midrule
artificial, 100 & 5000 & 11.922 && \textbf{12.265} & \textbf{849} && 12.315 & 825 && 12.304 & 832 \\
artificial, 500 & 5000 & 57.722 && \textbf{61.836} & \textbf{65} && 62.322 & 46 && 62.546 & 51 \\
artificial, 1000 & 5000 & 114.965 && \textbf{126.308} & 0 && 126.918 & 0 && 127.122 & 0 \\
\midrule
\midrule
real, 100 & 2500 & 23.473 && 26.012 & \textbf{569} && 26.084 & 568 && \textbf{25.955} &\textbf{569} \\
real, 500 & 2500 & 115.239 && 134.342 & 45 && 134.586 & \textbf{55} && \textbf{134.24} & 39 \\
real, 1000 & 2500 & 229.946 && \textbf{270.396} & 1 && 270.654 & \textbf{2} && 270.59 & 0 \\
\midrule
real, 100 & 5000 & 11.981 && \textbf{17.571} & \textbf{484} && 17.621 & 466 && 17.608 & 466 \\
real, 500 & 5000 & 57.865 && \textbf{93.875} & 85 && 94.549 & \textbf{86} && 94.186 & 82 \\
real, 1000 & 5000 & 115.227 && \textbf{193.08} & \textbf{29} && 193.658 & 27 && 193.578 &\textbf{29} \\
\bottomrule
\end{tabular}	
\vspace{0.2cm} %
\begin{tablenotes}
...
Firstly, it is immediately evident that the EA outperforms the previous heuristics, producing solutions using $t$ bins in all but one of the 12 subclasses. Fewer iterations of the EA were performed as $n$ and $W$ increaded, with the average number of iterations ranging from 300,000 to 1400 when $W=2500$ for artificial instances using 100 and 1000 items respectively. The corresponding figures for real instances when $W=5000$ were 470 and 65 iterations.

It can be seen that the EAs using the GGA and AGX$'$ recombination operators generate the best solutions overall. GGA consistently produces the best results for both artificial and real instance types when the larger bin size is used, where higher quality solutions contain 8.6 items per bin on average. \ea{dicuss why GGA works best for $W=5000$.}

On the other hand, when the smaller bin capacity $W=2500$ is used, where the best solutions average 4.3 items per bin, AGX$'$ can be see to produce the more optimal solutions. \ea{explain why AGX$'$ is best for smaller bin size.}

AGX generates the least favourable results, particularly for the real instance types, where it is outperformed by GGA and AGX$'$ in all 6 sub classes in regards to the average number of bins per solution $|\mathcal{S}|$. However, note that AGX yields the highest number of solutions containing $t$ bins in three of the subclasses, suggesting a high variance in the quality of solutions produced.
...
\begin{algorithmic}[1]
	...
	\While{time limit not reached}
		...
		\If{$\mathcal{S}^*$ is better than $\mathcal{S}_{bsf}$}
...
\noindent Firstly, a fixed number $p$ of solutions are produced using MFFR$^+$, and the bins of each solution are inserted into a set $\mathcal{B}^*$. The age of every new bin $b$ added to $\mathcal{B}^*$, $age[b]$, is initially set to 0. Then, the exact cover algorithm is applied to $\mathcal{B}^*$, generating an optimal solution $\mathcal{S}^*$, which, if better than the best-so-far solution $\mathcal{S}_{bsf}$, is stored as the new best-so-far solution. $\mathcal{B}^*$ is then adapted, with the age of bins used in $\mathcal{S}^*$ reset to zero, and the age of all other bins increased by one. Any bins in $\mathcal{B}^*$ whose age has reached the maximum are removed from the set. This process is repeated until a set time limit has been reached. 

\begin{comment}
{\color{myGreen}
\begin{itemize}[leftmargin=*]
	\item Cite \citet{blum2016}
