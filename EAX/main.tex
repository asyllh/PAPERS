...
Although the CMSA framework is simple, fewer iterations are performed in comparison to the EA within the same time frame. The recombination operators used in the EA are performed on only two parent solutions in a population, which in the worst-case would each comprise $|\mathcal{I}|$ bins, and simply consists of selecting bins from each parent. The local search procedure which is applied on a pair of partial solutions can be seen to take longer, however using AHC is also used which takes at most $O(n^2)$ time. On the other hand, \textsc{MinXC} is executed on the entire set of bins $\mathcal{B}$, which in some cases contains over 2900 bins and involves recursion and backtracking through all bins, and could take an impractical amount of time to complete. Thus, a time limit is set for the procedure, and the best solution found is taken as $\mathcal{S}^*$. In the event that the time limit is reached and \textsc{MinXC} has not found a single solution, $\mathcal{S}^*$ is set to $\mathcal{S}_{\textup{bsf}}$. This causes issues in many of the experiments. For example, in both artificial and real classes with $|\mathcal{I}| = 1000$ and $W =2500$, \textsc{MinXC} only returned a solution $\mathcal{S}^*$ in the first iteration of CMSA for all 50 instances, with no solutions being found within the time limit in all subsequent iterations. As a result, the average number of iterations for these classes were just 7 and 6.98 respectively. In comparison, the corresponding figures for the EA counterparts were 8603.66 and 5551.34 iterations.

{\color{myGreen}
\begin{itemize}[leftmargin=*]
	\item This issue (600 seconds reached and no solution found) also occurs for CMSA52, CMSA105, CMSR52 and CMSR105.
	\item (for CMSA102 and CMSR102) so the only solution found in these cases is the one found in the first iteration of CMSA, so there is no evolution of the pool of bins/set $\mathcal{B}$
	\item \textsc{MinXC} is applied onto the entire set $\mathcal{B}$, whereas only two solutions selected at random from population are used in each iteration of EA, so solutions containing high quality bins in pop have to wait until they are chosen.
	\item Solutions containing bad quality bins in EA bulk up the population and can only be removed is chosen which may take many iterations, however CMSA individual bins are removed and replaced frequently.
	\item Could have duplicate solutions in EA population or duplicate bins in different solutions, whereas in CMSA $\mathcal{B}$ does not have any duplicates.
	\item Can merge characteristics from EA with CMSA, CMSA is simple set up, better because all bins assessed simultaneously rather than only two solutions at a time, but only MFFR$^+$ used, could use mutation and local search from EA to produce better quality bins for $\mathcal{B}$. 
	\item Also could limit the size of $\mathcal{B}$ so that more iterations can be performed and solutions can be found in each iteration, resulting in an evolution of the set $\mathcal{B}$.
