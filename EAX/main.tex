...
\centering
\caption{Results obtained from the EA using the GGA, AGX, and AGX$'$ recombination operators, and from the MFFD$^+$ heuristic. Figures in bold indicate the best results for each instance class. Asterisks indicate statistical significance at $\leq 0.05 (^{*})$ and $\leq 0.01 (^{**})$ according to a two-tailed paired t-test and two-tailed McNemar's test for the $|\mathcal{S}^*|$ and $\%t$ columns respectively. \ea{stats tests only on xOvers, not MFFD$^+$.}}
\scriptsize
\begin{threeparttable}
...
\begin{algorithmic}[1]
	\State $\mathcal{S}^* \gets \emptyset$, $\mathcal{B} \gets \emptyset$
	\While{time limit not reached}
		\For{$i\gets 1 \To p$}
			...
			\EndFor
		\EndFor
		\State $\mathcal{S}_{\textup{x}} \gets$ \textsc{MinXC}($\mathcal{B}$)
		\If{$\mathcal{S}_{\textup{x}}$ is better than $\mathcal{S}^*$}
			\State $\mathcal{S}^* \gets \mathcal{S}_{\textup{x}}$
		\EndIf
		\ForAll{$b \in \mathcal{B}$}
			\If{$b \in \mathcal{S}_{\textup{x}}$}
				\State $age[b] \gets 0$
			\ElsIf{$b \notin \mathcal{S}_{\textup{x}}$}
