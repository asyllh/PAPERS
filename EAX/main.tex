...
\begin{subequations}
	\begin{alignat}{3}
		\text{minimise  } &\sum_{i \in M} x_i & \\[3pt]
		\text{subject to  } &\sum_{i \in M} b_{ij} x_i = 1 &\quad &\forall \hspace{1mm} j \in N \\[3pt]
		&x_i \in \{0,1\} & &\forall \hspace{1mm} i \in M
	\end{alignat}
\end{subequations}
...
\label{sec:xpress}
\begin{itemize}[leftmargin=*]
	\item $n$ vertices, one for each score width - depot is vertex 0 (base vertex, universal vertex), $N = \{1,2,\dotsc,n\}$
	\item $m$ vehicles, one for each `tour'. Set $m = n\backslash 2$ as this is the maximum number of tours possible, equivalent to each item having its own bin, $M = \{1,2,\dotsc,m\}$
	\item $d_{ij}$ is the distance travelled from vertex $i$ to vertex $j$ - equivalent to the weight of edge $(i,j)$ which is the width of the item
	\item $D$ is the maximum allowed distance travelled for each vehicle $k$ - equivalent to $W$, the bin capacity. 
	\item $a_{ij}$ = 1 if edge exists between $i$ and $j$, 0 otherwise
	\item $p{i}$ is the partner of $i$
\end{itemize}

\begin{subequations}
	\begin{alignat}{3}
	\text{minimise  } &\sum_{k=1}^m \sum_{i=1}^n x_{i0}^k & \\[3pt]
	\text{subject to  } &\sum_{k=1}^m \sum_{i=0}^n x_{ij}^k = 1 &\quad &\forall \hspace{1mm} j \in N \\[3pt]
	&\sum_{k=1}^m \sum_{j=0}^n x_{ij}^k = 1 &\quad &\forall \hspace{1mm} i \in N \\[3pt]
	&\sum_{i=0}^n x_{it}^k - \sum_{j=0}^n x_{tj}^k = 0 &\quad &\forall \hspace{1mm} k \in M, \hspace{1mm} t \in N \\[3pt]
	&\sum_{k=1}^m (x_{i,p(i)}^k + x_{p(i),i}^k) = 1 &\quad &\forall \hspace{1mm} i \in N \\[3pt]
	&\sum_{i=0}^n \sum_{j=0}^n d_{ij} x_{ij}^k \leq D &\quad &\forall \hspace{1mm} k \in M \\[3pt]
	&\sum_{i=1}^n x_{i0}^k \leq 1 &\quad &\forall \hspace{1mm} k \in M \\[3pt]
	&\sum_{j=1}^n x_{0j}^k \leq 1 &\quad &\forall \hspace{1mm} k \in M \\[3pt]
	&x_{ij}^k \leq a_{ij} &\quad &\forall \hspace{1mm} i,j \in N \cup \{0\}, \hspace{1mm} k \in M \\[3pt]
	&x_{ij}^k \in \{0,1\} & &\forall \hspace{1mm} i,j \in N \cup \{0\}, \hspace{1mm} k \in M
	\end{alignat}
\end{subequations}
