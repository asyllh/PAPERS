...
Otherwise, the edges in $R_1$ are removed from the list of edges, and the algorithm initiates an iterative procedure to find new sets. In each iteration, BR starts the search from the beginning of the list to find edges to start a new set, with the additional condition that the set must overlap with a set in $\mathcal{R}$. If such a set is created, it is added to $\mathcal{R}$, and BR continues through the list of edges in search of another overlapping set. Once the penultimate edge has been assessed, edges that are in sets in $\mathcal{R}$ are removed from the list.

This process continues until one of the following events occurs:
\begin{enumerate}[label={(\roman*)}]
	\item If at any point $\mathcal{R}$ contains at least one edge from every component of $G'$, the procedure immediately terminates. The connecting procedure is used on all sets in $\mathcal{R}$ to combine the components of $G'$. Feasible.
	\item If no new sets were created during an iterations, then no more sets exist, and the sets currently in $\mathcal{R}$ cannot combine all components of $G'$. Infeasible.
	\item If the list only one edge, not enough edges to create a new set, and the sets currently in $\mathcal{R}$ cannot combine all components of $G'$. Infeasible.
\end{enumerate}
