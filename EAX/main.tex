...
CP operates on each set $R^*_i \subset \mathcal{R}^*$ as follows: for each edge in $R^*_i$ in turn, the edge from $R \backslash R'$ connecting the smaller vertex of the edge to the larger vertex of the next edge is added to $R'$ (for the last edge in $R^*_i$, take the first edge to be the next edge). The edges in $R^*_i$ that appear in $R'$ are then removed, resulting in $n+1$ edges in $R'$ that form an alternating Hamiltonian cycle on $G'$ with the edges in $B$. An example of this is shown in Figure~\ref{fig:bcr}, where $\mathcal{R}^*$ comprises a single set $R^*_1 = \{(v_3, v_{12}),(v_4, v_{11}), (v_5, v_{10})\}$. 
...
\begin{figure}[H]	
	...
	\end{subfigure}
	\caption{(a) Sorted list $\mathcal{L}$ of edges from $R'$, and set $\mathcal{R}^* = \{R^*_1\}$ containing three edges produced by BCR. Dashed lines show the edges from $R\backslash R'$ obtained using CP; (b) and (c) edges from $R\backslash R'$ linking the components, replacing edges in $R'$ to form a single alternating Hamiltonian cycle; (d) removing universal vertices and incident edges results in an alternating Hamiltonian path corresponding to a solution $\mathcal{T}$.}
	\label{fig:bcr}
\end{figure}
...
\begin{itemize}[leftmargin=*]
	\item 2-factor $G'$.
	\item ``Matching'' edge from $R'$, distinguish from $B$ as $B$ is also a matching, then can use matching edge rather than red edge.
	\item State red and blue edges in figures.
