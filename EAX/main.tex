...
\begin{abstract}
\note{Type of packing problem in which order and orientation of items is important. Paper investigates heuristics, EA, postoptimisation.} \alert{make sure it says ``vicinal sum constraint'', not ``vicinal score constraint''.}
\end{abstract}	
\end{frontmatter}
...
\noindent where \note{$A(S_j) = \sum_{i \in S_j} w_i$ is the total width of all items on a strip $S_j$.} This function produces a fitness value between 0 and 1. The higher the fitness, the better the quality of the packing of items in the solution.
...
\begin{itemize}[leftmargin=*]
	...
	\item As an example, use fitness values from example solutions created using FFD/MFFD/MFFD$^+$ from Heuristics section figures.
\end{itemize}
}
