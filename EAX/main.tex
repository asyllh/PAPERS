...
\begin{figure}[h]
	\centering	
	\subfloat[BPP]{\includestandalone[width=0.35\textwidth]{figures/bpp}
	\label{fig:bpp}}
	\vspace{3mm}
	\subfloat[SCPP]{\includestandalone[width=0.35\textwidth]{figures/scpp}
	\label{fig:scpp}}
...
\label{sec:ahc}
\noindent In this section, we focus on the SubSCP, which involves packing items into a single bin. To begin, let us first consider the following sequencing problem originally defined by \cite{hawa2018}:
...
\begin{figure*}[h]	
	...
	\label{fig:mpscycle}}
	\hspace{3mm}
	\subfloat[]{\includestandalone[width=0.65\textwidth]{figures/solutionpath}
...
\begin{figure}[H]	
	\centering
	\subfloat[MFFD]{\includestandalone[width=0.33\textwidth]{figures/mffd}
		\label{fig:mffd}}
	\hspace{15mm}
	\subfloat[MFFD$^+$]{\includestandalone[width=0.33\textwidth]{figures/mffdplus}
		\label{fig:mffdplus}}
...
\begin{figure*}[h!]	
	\centering
	\subfloat[GGA]{\includestandalone[width=0.3\textwidth]{figures/gga}
		\label{fig:gga}}
	\hspace{5mm}
	\subfloat[AGX]{\includestandalone[width=0.3\textwidth]{figures/agx}
		\label{fig:agx}}
	\hspace{5mm}
	\subfloat[AGX$'$]{\includestandalone[width=0.3\textwidth]{figures/agxdash}
		\label{fig:agxdash}}
...
This method is based on the dominance criterion of \cite{martello1990l} for the BPP. Variations of this method can be seen in \cite{lewis2009, lewis2017, falkenauer1996, levine2004}; however the addition of the vicinal sum constraint will often result in fewer changes than seen in these previous implementations.
...
\ea{Also observe the coefficient of variation is less for artificial instances than for real instances. This could be due to the theoretical minimum $t$ being less accurate for real instances because of the lack of diversity in item dimensions, i.e. items are weakly heterogeneous.}
...
\begin{figure}[h!]	
	\centering
	\includegraphics[width=0.3\textwidth]{figures/AlgCMSAcrop}
...
\EndIf
\EndFor
\If{$m(v_i) = \textsc{null} \AAnd i \neq 1 \AAnd m(v_{i-1}) \neq \textsc{null} \AAnd \{v_{i-1}, p(v_i)\} \in R$}
