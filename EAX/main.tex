...
It is immediately evident that all EA versions outperform the MFFD$^+$ heuristic, with superior averages in all cases. Fewer iterations of the EA were performed as $|\mathcal{I}|$ and $W$ increased, with the average number of iterations ranging from 360,000 to 1200 when $W=2500$ for artificial instances using 100 and 1000 items respectively. The corresponding figures for real instances using $W=5000$ were 55,000 and 65 iterations. \ea{This is because more bins and more items per bin on average results in local search procedure taking longer as there are more applications of AHC.}

It can be seen that the EAs using the GGA and AGX$'$ recombination operators generate the best solutions overall. \ea{The different recombination operators seem to perform well with different types of problem instances.} GGA consistently produces the best results for both artificial and real instances using the larger bin size, where higher quality solutions contain approximately 8.6 items per bin on average. \ea{It seems that lack of bias in choice of bins to inherit allows more of the solution space to be explored.} The random selection of bins from parents results in entire solutions being evaluated, rather than individual bins, thus good combinations of bins can be found to form better solutions.

On the other hand, when $W = 2500$ (where the best solutions average 4.3 items per bin) AGX$'$ can be see to produce \ea{more optimal solutions (is this true?)} for artificial instances. By selecting bins containing the most items, AGX$'$ aims to build solutions comprising fewer yet fuller bins, thus AGX$'$ is focused on higher quality individual bins as opposed to entire solutions.

\ea{The difference between $|\mathcal{S}|$ and $t$ increase as $|\mathcal{I}|$ increases, more so for larger bin size. This could be because the EA is less accurate as $|\mathcal{I}|$ increases, and also fewer iterations of EA means less evolution of the population.}

\ea{Difference in coefficient of variation between real and artificial instances. Is $t$ worse/less accurate for real instances due to lack of item diversity?}
...
Ideally, the entire set of feasible bins $\mathcal{F}$ would be used to form $\mathcal{B}$, however since the cardinality of $\mathcal{F}$ has the potential to grow at an exponential rate in relation to the problem size, it may not be possible to produce $\mathcal{B}$, let alone find the \cmsa{smallest} exact cover in a feasible amount of time. One option here would be to implement a method of finding \cmsa{a minimum cardinality?} an exact cover within the Construct, Merge, Solve \& Adapt (CMSA) algorithm of \citet{blum2016}. Rather than having a population of full solutions as used in our EA, CMSA operates on the set $\mathcal{B}$ of individual feasible bins.

\begin{figure}[h!]	
	\centering
	\includegraphics[width=\textwidth]{figures/AlgorithmCMSA}
	\caption{\cmsa{alg cmsa.}}	
	\label{fig:algcmsa}
\end{figure}
