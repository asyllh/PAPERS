...
%			\State $R' \gets R' \cup \{\{v_i, m(v_i)\}\}$
		\EndIf
	\EndFor
	\State \ahc{then need to return set $R'$ containing all matching edges}
\end{algorithmic}
\label{alg:mcm}	
\end{algorithm}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Issue caused using the original procedure to find suitable edge subsets, where the collection $\mathcal{R}''$ covers all components of $G'$, but the bridges obtained from the edge subsets in $\mathcal{R}''$ form two components, as opposed to a single alternating Hamiltonian cycle.}	
